  ...
  <div class="content-wrapper">
    ...
    <section class="content">
      ...
      <div class="box">
        ...
        <!-- /.box-header -->
        <form class="form-horizontal" method="POST" action="{{ route('kerjasama-luar-negeri.store') }}" enctype="multipart/form-data">
          {{ csrf_field() }}
          <div class="box-body">
            <div class="col-sm-6">
              <div class="form-group{{ $errors->has('id_katln') ? ' has-error' : '' }}">
                ...
                <div class="col-sm-8">
                  <select class="form-control" id="id_katln" name="id_katln" required>
                    ...
                    @foreach($katln as $ln)
                      <option value="{{ $ln->id }}" {{ old('id_katln') == $ln->id ? 'selected' : '' }}>{{ $ln->name }}</option>
                    @endforeach
                  </select>
                ...
                <div class="col-sm-8">
                  <select class="form-control" id="id_katjenisln" name="id_katjenisln" required>
                    ...
                    @foreach($katjenisln as $jenisln)
                      <option value="{{ $jenisln->id }}" {{ old('id_katjenisln') == $jenisln->id ? 'selected' : '' }}>{{ $jenisln->name }}</option>
                    @endforeach
                  </select>
