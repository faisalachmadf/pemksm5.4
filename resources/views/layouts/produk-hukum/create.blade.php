  ...
  <div class="content-wrapper">
    ...
    <section class="content">
      ...
      <div class="box">
        ...
        <!-- /.box-header -->
        <form class="form-horizontal" method="POST" action="{{ route('produk-hukum.store') }}" enctype="multipart/form-data">
          {{ csrf_field() }}
          <div class="box-body">
            <div class="form-group{{ $errors->has('nama') ? ' has-error' : '' }}">
              ...
              <div class="col-sm-4">
                <input type="text" class="form-control" id="nama" name="nama" value="{{ old('nama') }}" placeholder="Nama Produk Hukum" required>
              ...
              <div class="col-sm-4">
                <select class="form-control" id="id_kathukum" name="id_kathukum" required>
                  ...
                  @foreach($kathukum as $hukum)
                    <option value="{{ $hukum->id }}" {{ old('id_kathukum') == $hukum->id ? 'selected' : '' }}>{{ $hukum->name }}</option>
                  @endforeach
                </select>
