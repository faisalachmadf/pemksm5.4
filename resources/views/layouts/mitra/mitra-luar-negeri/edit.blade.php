  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li><a href="{{ route('mitra-luar-negeri.index') }}">Mitra</a></li>
    ...
    <section class="content">
      ...
      <div class="box">
        ...
        <!-- /.box-header -->
        <form class="form-horizontal" method="POST" action="{{ route('mitra-luar-negeri.update', @$mitraln->slug) }}" enctype="multipart/form-data">
          {{ csrf_field() }}
          {{ method_field('PUT') }}
          <input type="hidden" name="id" value="{{ @$mitraln->id }}">
          <div class="box-body">
          @if($mitraln)
            <div class="form-group{{ $errors->has('judul') ? ' has-error' : '' }}">
              ...
              <div class="col-sm-4">
                <input type="text" class="form-control" id="judul" name="judul" value="{{ old('judul', $mitraln->judul) }}" placeholder="Judul" required>
            ...
            <div class="form-group{{ $errors->has('id_katmitraln') ? ' has-error' : '' }}">
              <label for="id_katmitraln" class="col-sm-4 control-label">Kategori Mitra Kerjasama luar Negeri<span class="required">*</span> :</label>

              <div class="col-sm-4">
                <select class="form-control" id="id_katmitraln" name="id_katmitraln" required>
                  <option value="">Pilih Kategori mitra Kerjasama</option>
                  @foreach($katmitraln as $kategori)
                    <option value="{{ $kategori->id }}" {{ old('id_katmitraln', $mitraln->id_katmitraln) == $kategori->id ? 'selected' : '' }}>{{ $kategori->name }}</option>
                  @endforeach
                </select>
            ...
            <div class="form-group{{ $errors->has('isi') ? ' has-error' : '' }}">
              ...
              <div class="col-sm-12">
                <textarea class="form-control" id="isi" name="isi" placeholder="Sumarry" required>{{ old('isi', $mitraln->isi) }}</textarea>
